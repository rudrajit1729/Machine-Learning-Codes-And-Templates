...
wcss = []
for i in range (1, 11):#We will deal with 10 clusters at most
...
plt.ylabel('WCSS')
plt.show()#From graph get the elbow point --> optimal no. of clusters

#Applying KMeans to the mall dataset(K = 5)
kmeans = KMeans(n_clusters = 5, init = 'k-means++', n_init = 10,
                    max_iter = 300, random_state = 0)
y_kmeans = kmeans.fit_predict(X)# For each obs. it returns which clusters it belong to
